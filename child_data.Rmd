...
```{r, eligibility}
child_eligibility1 <- child_eligibility %>%
...
```{r, eligibility}
plot_elig <- ggplot(child_eligibility1, aes(year, n, 
                                            fill = elig_code)) +
...
plot_foster <- ggplot(child_data_a,
                      aes(x = medi_fost, y = percent, fill = catebory)) +
...
plot_rescreened <-ggplot(child_ref_rescreen,
                         aes(x = ref_age_class, y = n, fill = outcome)) +
  geom_col(width = 0.5) +
  scale_fill_manual(values=c("#CC79A7", "#68a4bd")) +
  theme_economist() +
  theme(axis.title.x = element_blank(),
...
#str(child_ref_outcome)
```


```{r, secondary ref_source}
...
write.table(child_site_center, file = "child_site_center.xlsx")
...
```{r}
center_plot <- ggplot(child_site_center, aes(reorder(Placement, Number), Number,
                                             fill = Region)) +
  geom_col(width = 0.5) +
  theme_economist() +
  scale_fill_manual(values=c("#4c4c4c", "#86BB8D", "#B22222", "#ff9900",
